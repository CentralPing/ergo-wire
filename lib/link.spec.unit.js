...
describe('[Boundary] lib/link', () => {
  ...
  it('parseLinkHeader last-wins when duplicate same-rel link-values appear', () => {
    const parsed = parseLinkHeader('</a>; rel="next", </b>; rel="next", </c>; rel="prev"');
    assert.equal(parsed.size, 2);
    assert.equal(parsed.get('next')?.href, '/b');
    assert.equal(parsed.get('prev')?.href, '/c');
  });
});
