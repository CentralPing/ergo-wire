...
describe('[Boundary] lib/pagination', () => {
  ...
  for (const {input, wire} of vectors.serializeCursor) {
    it(`serializeCursorParams ${JSON.stringify(input)}`, () => {
      const result = serializeCursorParams(input);
      assert.equal(result.limit, wire.limit);
      if (wire.cursor === undefined) {
        assert.equal('cursor' in result, false);
      } else {
        assert.equal(result.cursor, wire.cursor);
      }
      assert.deepEqual(parseCursorParams(result), {
        cursor: wire.cursor,
        limit: wire.limit
      });
    });
  ...
  it('serializeCursorParams validates limit and cursor', () => {
    assert.throws(() => serializeCursorParams({limit: 0}), TypeError);
    assert.throws(() => serializeCursorParams({limit: -1}), TypeError);
    assert.throws(() => serializeCursorParams({limit: 1.5}), TypeError);
    assert.throws(() => serializeCursorParams({limit: 10, cursor: 123}), TypeError);
    assert.throws(() => serializeCursorParams({limit: 10, cursor: null}), TypeError);
  });

  it('serializeCursorParams preserves empty cursor string', () => {
    const result = serializeCursorParams({limit: 10, cursor: ''});
